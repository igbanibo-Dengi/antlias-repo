...
export const tankStatusEnum = pgEnum("tank_status", [
  "active",
  "maintenance",
  "decommissioned",
]);
...
export const users = pgTable(
  ...
  (table) => ({
    ...
    tenantActiveIdx: index("users_tenant_active_idx").on(
      table.tenantId,
      table.isActive,
    ),
...
export const employees = pgTable(
  ...
  (table) => ({
    userTenantBranchUnique: uniqueIndex(
      "employees_user_tenant_branch_unique",
    ).on(table.userId, table.tenantId, table.branchId),
...
export const tanks = pgTable(
  "tanks",
  {
    ...
    fuelType: text("fuel_type").notNull(),
    currentLevel: decimal("current_level", {
      precision: 10,
      scale: 2,
    }).notNull(),
  ...
  (table) => ({
    ...
    fuelStatusIdx: index("tanks_fuel_status_idx").on(
      table.fuelType,
      table.status,
    ),
    numberUnique: uniqueIndex("tanks_number_unique").on(
      table.tenantId,
      table.branchId,
      table.tankNumber,
    ),
...
export const transactions = pgTable(
  ...
  (table) => ({
    branchDateIdx: index("transactions_branch_date_idx").on(
      table.branchId,
      table.createdAt,
    ),
...
export const deliveryLogs = pgTable(
  ...
  (table) => ({
    branchDateIdx: index("deliveries_branch_date_idx").on(
      table.branchId,
      table.deliveryDate,
    ),
